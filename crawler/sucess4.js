...
class DomesticCrawler4 {
  ...
  async mcst() {
    const url =
      "https://www.mcst.go.kr/kor/s_notice/press/pressList.jsp?pCurrentPage=1&pSearchType=&pSearchWord=&pTypeDept=";
    const resp = await this.client.get(url);
    const $ = cheerio.load(resp.data);
    return {
      date0: this._getDates($)[2],
      date1: this._getDates($)[6],
      date2: this._getDates($)[10],
      date3: this._getDates($)[14],
      date4: this._getDates($)[18],
...
module.exports = DomesticCrawler4;
